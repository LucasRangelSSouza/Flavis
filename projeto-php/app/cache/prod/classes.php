...
public function generate($name, $parameters = array(), $referenceType = self::ABSOLUTE_PATH, $perfilLogado);
}
}
namespace Symfony\Component\Routing\Generator
{
...
//class Router //\Symfony\Component\Routing\Router
namespace Symfony\Component\Routing
{
    ...
    // extends Sonata\AdminBundle\Admin\Admin
    // extends Symfony\Component\DependencyInjection\ContainerAware
    // extends Symfony\Bundle\FrameworkBundle\Controller\Controller
    // extends Sonata\AdminBundle\Admin\AdminInterface
...
//        use Symfony\Component\Routing\Router as BaseRouter;
    ...
    class Router implements RouterInterface, RequestMatcherInterface
    {
        ...
        //responsavel pela função ao logar
        //responsavel pela função list do botão Exibir
        //responsavel pela função edit do botão Editar
        //responsavel pela função create do botão Novo
        //responsavel pela função delete do botão Deletar
        public function generate($name, $parameters = array(), $referenceType = self::ABSOLUTE_PATH, $perfilLogado)
        {
            //Criar rotina para pegar o perfil
        //request.CRITICAL:
        // Uncaught PHP Exception Symfony\Component\Debug\Exception\FatalThrowableError:
        // "Type error: Too few arguments to function Symfony\Component\Routing\Router::generate(), 3 passed
        // in C:\Apache2.4\htdocs\flavis\projeto-php\vendor\symfony\symfony\src\Symfony\Bridge\Twig\Extension\RoutingExtension.php
        // on line 45 and exactly 4 expected" at C:\Apache2.4\htdocs\flavis\projeto-php\app\cache\prod\classes.php line 1271
        // {"exception":"[object] (Symfony\\Component\\Debug\\Exception\\FatalThrowableError(code: 0): Type error:
        // Too few arguments to function Symfony\\Component\\Routing\\Router::generate(), 3 passed
        // in C:\\Apache2.4\\htdocs\\flavis\\projeto-php\\vendor\\symfony\\symfony\\src\\Symfony\\Bridge\\Twig\\Extension\\RoutingExtension.php
        // on line 45 and exactly 4 expected at C:\\Apache2.4\\htdocs\\flavis\\projeto-php\\app\\cache\\prod\\classes.php:1271)"} []
        //    $admin = $this;
//            echo 'Rastreabilidade! Método generate em classes:Router - perfil logado: '.$perfilLogado.'                             ';
            return $this->getGenerator()->generate($name, $parameters, $referenceType, $perfilLogado);
        }
        ...
        //responsavel pela função ao logar
        //responsavel pela função show do botão Exibir (monta o endereço com o perfil mas da erro ao abrir)
        //responsavel pela função edit do botão Editar (monta o endereço com o perfil mas da erro ao abrir)
        //responsavel pela função create do botão Novo (monta o endereço com o perfil mas da erro ao abrir)
        //responsavel pela função delete do botão Deletar (monta o endereço com o perfil mas da erro ao abrir)
        public function matchRequest(Request $request)
        {
...
class UrlMatcher implements UrlMatcherInterface, RequestMatcherInterface//extends ContainerAware
{
...
public function generateUrl($name, array $parameters = array(), $absolute = false)
{
    ...
    $perfilLogado = $perfilToFilter;
//    echo 'Rastreabilidade! Método generateUrl em classes:Admin - perfil logado: '.$perfilLogado.'                             ';
    return $this->routeGenerator->generateUrl($this, $name, $parameters, $absolute, $perfilLogado);
}
public function generateMenuUrl($name, array $parameters = array(), $absolute = false)
{
...
public function buildBreadcrumbs($action, MenuItemInterface $menu = null)
{
    ...
    $perfilLogado = $perfilToFilter;
//    echo 'Rastreabilidade! Método buildBreadcrumbs em classes:Admin - perfil logado: '.$perfilLogado.'                             ';
    if (!$menu) {
        $menu = $this->menuFactory->createItem('root');
        $menu = $menu->addChild(
        $this->trans($this->getLabelTranslatorStrategy()->getLabel('dashboard','breadcrumb','link'), array(),'SonataAdminBundle'),
        array('uri'=> $this->routeGenerator->generate('sonata_admin_dashboard', $parameters = array(), $absolute = false, $perfilLogado))
        );
    }
    $menu = $menu->addChild(
...
{
public function generateUrl(AdminInterface $admin, $name, array $parameters = array(), $absolute = false, $perfilLogado);
public function generateMenuUrl(AdminInterface $admin, $name, array $parameters = array(), $absolute = false);
public function generate($name, array $parameters = array(), $absolute = false, $perfilLogado);
...
use Symfony\Component\Routing\RouterInterface;
    use Symfony\Component\DependencyInjection\ContainerAware;
class DefaultRouteGenerator extends ContainerAware implements RouteGeneratorInterface
{
private $router;
private $cache;
...
public function generate($name, array $parameters = array(), $absolute = fals, $perfilLogado)
{
//    $container = $this->getAdminPool()->getContainer();//Call to a member function getContainer() on null
//    $em = $container->get('doctrine.orm.default_entity_manager');
//    $colaboradores = $em->getRepository('AppBundle:Colaborador')->findAll(array(),array('nome' => 'ASC'));
//    $colaboradorLogado = $container->get('security.context')->getToken()->getUser();
//
...
//    }
//    $perfilLogado = $perfilToFilter;
//    echo 'Rastreabilidade! Método generate em classes:DefaultRouteGenerator - perfil logado: '.$perfilLogado.'                             ';
    return $this->router->generate($name, $parameters, $absolute, $perfilLogado);
}
public function generateUrl(AdminInterface $admin, $name, array $parameters = array(), $absolute = false, $perfilLogado)
{
$arrayRoute = $this->generateMenuUrl($admin, $name, $parameters, $absolute);
//    $container = $admin->getConfigurationPool()->getContainer();//Call to a member function getContainer() on null
////    $container = $this->getAdminPool()->getContainer();//Call to a member function getContainer() on null
//    $em = $container->get('doctrine.orm.default_entity_manager');
//    $colaboradores = $em->getRepository('AppBundle:Colaborador')->findAll(array(),array('nome' => 'ASC'));
//    $colaboradorLogado = $container->get('security.context')->getToken()->getUser();
...
//    }
//    $perfilLogado = $perfilToFilter;
//    echo 'Rastreabilidade! Método generateUrl em classes:DefaultRouteGenerator - perfil logado: '.$perfilLogado.'                             ';
    return $this->router->generate($arrayRoute['route'], $arrayRoute['routeParameters'], $arrayRoute['routeAbsolute'], $perfilLogado);
}
public function generateMenuUrl(AdminInterface $admin, $name, array $parameters = array(), $absolute = false)
{
...
public function editAction($id, $mode)
{
    ...
    $perfilLogado = $perfilToFilter;
//    echo 'Rastreabilidade! Método editAction em classes:Pixlr - perfil logado: '.$perfilLogado.'                             ';
    $parameters = array('s'=>'c','referrer'=> $this->referrer,'exit'=> $this->router->generate('sonata_media_pixlr_exit', array('hash'=> $hash,'id'=> $media->getId()), true, $perfilLogado),'image'=> $provider->generatePublicUrl($media,'reference'),'title'=> $media->getName(),'target'=> $this->router->generate('sonata_media_pixlr_target', array('hash'=> $hash,'id'=> $media->getId()), true, $perfilLogado),'locktitle'=> true,'locktarget'=> true,
    );
