...
class CacheManager
{
    ...
    public function generateUrl($path, $filter, array $runtimeConfig = array())
    {
        ...
        if (empty($runtimeConfig)) {
            $filterUrl = $this->router->generate('liip_imagine_filter', $params, true, $perfilLogado);
        } else {
            $params['filters'] = $runtimeConfig;
            $params['hash'] = $this->signer->sign($path, $runtimeConfig);

            $filterUrl = $this->router->generate('liip_imagine_filter_runtime', $params, true, $perfilLogado);
        }
