...
//        echo 'Rastreabilidade! Método editAction em Pixlr - perfil logado: '.$perfilLogado.'                             ';
        $parameters = array(
            's'          => 'c', // ??
            'referrer'   => $this->referrer,
            'exit'       => $this->router->generate('sonata_media_pixlr_exit', array('hash' => $hash, 'id' => $media->getId()), true, $perfilLogado),
            'image'      => $provider->generatePublicUrl($media, 'reference'),
            'title'      => $media->getName(),
            'target'     => $this->router->generate('sonata_media_pixlr_target', array('hash' => $hash, 'id' => $media->getId()), true, $perfilLogado),
            'locktitle'  => true,
            'locktarget' => true,
        );
